...
function constructCalendarGridValue(infoDate, focusDate, selectedDate, viewedMonthDate) {
	return {
		date: infoDate,
		isFocusDate: dateIsEqualYMD(infoDate, [focusDate, selectedDate].find((testDate) => testDate && isValid(testDate)) ?? /* @__PURE__ */ new Date()),
...
function calendarGrid(focusDate, selectedDate) {
	...
	const viewedMonthDate = focusDate && isValid(focusDate) ? focusDate : /* @__PURE__ */ new Date();
	...
	for (let loopDate = add(firstOfMonth, { days: -1 * startDayOfWeek + 1 }); dateIsEqualYM(loopDate, viewedMonthDate) <= 0 || calendarGridMonth[calendarGridMonth.length - 1]?.length !== 7; loopDate = add(loopDate, { days: 1 })) {
...
function moveCurrentValueFocus(calendarInputId, oldDate, dateFormat, duration) {
	const newDate = add(oldDate && isValid(oldDate) ? oldDate : /* @__PURE__ */ new Date(), duration);
...
function CalendarInput({ className, dateFormat = "MM/dd/yyyy", errorMessage, id, innerRef, isDisabled, isHidden, isRequired, label, labelClassName, onChange, shouldSetFocusOnMount, showTodayButton, value, wrapperClassName, ...rest }) {
	...
	const calendarMonthDate = currentValueDateInternal && isValid(currentValueDateInternal) ? currentValueDateInternal : /* @__PURE__ */ new Date();
	const calendarGridValues = useMemo(() => calendarGrid(currentValueDateInternal, currentValueDate), [currentValueDateInternal, value]);
	const onDownArrowPress = useOnKeyUp("ArrowDown", useCallback(() => setCurrentValueDateInternal((date) => moveCurrentValueFocus(calendarInputId, date, dateFormat, { weeks: 1 })), []), true);
	const onUpArrowPress = useOnKeyUp("ArrowUp", useCallback(() => setCurrentValueDateInternal((date) => moveCurrentValueFocus(calendarInputId, date, dateFormat, { weeks: -1 })), []), true);
	const onLeftArrowPress = useOnKeyUp("ArrowLeft", useCallback(() => setCurrentValueDateInternal((date) => moveCurrentValueFocus(calendarInputId, date, dateFormat, { days: -1 })), []), true);
	const onRightArrowPress = useOnKeyUp("ArrowRight", useCallback(() => setCurrentValueDateInternal((date) => moveCurrentValueFocus(calendarInputId, date, dateFormat, { days: 1 })), []), true);
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [
			/* @__PURE__ */ jsxs("label", {
				...
				children: [label, isRequired ? /* @__PURE__ */ jsx(RequiredStar, {}) : null]
			}),
			/* @__PURE__ */ jsxs("div", {
				...
				children: [/* @__PURE__ */ jsxs("div", {
					...
					children: [
						/* @__PURE__ */ jsxs("div", { children: [shouldSetFocusOnMount ? /* @__PURE__ */ jsx("div", {
							...
							onClick: () => setCurrentValueDateInternal((draftDate) => {
								const newDate = add(draftDate && isValid(draftDate) ? draftDate : /* @__PURE__ */ new Date(), { months: -1 });
						...
						/* @__PURE__ */ jsx("div", {
							...
							children: format(calendarMonthDate, "MMMM")
						}),
						/* @__PURE__ */ jsx("div", { children: /* @__PURE__ */ jsx(IconButton, {
							className: "icon-button--small1x icon-button--borderless",
							icon: /* @__PURE__ */ jsx("span", {
					...
					children: [
						/* @__PURE__ */ jsx("div", { children: /* @__PURE__ */ jsx(IconButton, {
							...
							isDisabled,
							onClick: () => setCurrentValueDateInternal((draftDate) => {
								const newDate = add(draftDate && isValid(draftDate) ? draftDate : /* @__PURE__ */ new Date(), { years: -1 });
			...
			/* @__PURE__ */ jsxs("div", {
				...
				}), calendarGridValues.map((weekGridValues, weekGridValuesIndex) => /* @__PURE__ */ jsx("div", {
					className: "calendar-input__row",
					role: "row",
					children: weekGridValues.map((cellGridValue) => {
						...
						return /* @__PURE__ */ jsx(Button, {
							...
							children: /* @__PURE__ */ jsx("span", {
								"aria-label": `${format(cellGridValue.date, "EEEE MMMM do yyyy")}. Press return to select date.`,
			...
			showTodayButton ? /* @__PURE__ */ jsx("div", {
				...
				children: /* @__PURE__ */ jsx("button", {
					...
					onClick: () => {
						...
						onChange?.(format(/* @__PURE__ */ new Date(), dateFormat));
					},
					tabIndex: isHidden ? -1 : 0,
					type: "button",
					children: "Today"
...
function CharacterCount({ className, id, maxLength, text }) {
	...
	const displayMessage = useMemo(() => overLimit ? `${charactersOver} character${trailingS(charactersOver)} over the limit` : `${charactersLeft} character${trailingS(charactersLeft)} left`, [
		charactersLeft,
		charactersOver,
		overLimit
	]);
	...
	return maxLength !== void 0 && maxLength !== null ? /* @__PURE__ */ jsx("div", {
...
var ComboBoxOptionGroupContext = createContext("");
//#endregion
...
function ComboBoxOption({ children, className, isDisabled, identifiesWithOptionGroupId, isStatic, isHidden, label, value, ...rest }) {
	...
	const onEnterKeyPress = useOnKeyUp("Enter", useCallback(() => {
		if (!isDisabled) {
			onChange(value);
	...
	const onCancelKeyPress = useOnKeyUp("Escape", useCallback(() => setComboBoxContext((draftCombBoxContext) => {
	...
	const onDownArrowPress = useOnKeyUp("ArrowDown", useCallback(() => setComboBoxContext((draftContext) => moveComboBoxSelectionDown(draftContext, multiSelectContext)), [multiSelectContext, setComboBoxContext]), true);
	useEffect(() => {
	...
	return isVisible ? /* @__PURE__ */ jsx("li", {
...
function CombBoxListBox({ allowCustomEntry, ariaLabelledById, children, id, popupReferenceElement, portalTarget = null }) {
	...
	const listBox = /* @__PURE__ */ jsxs("ul", {
		...
		children: [
			children,
			!optionsFilteredWithoutGroupLabels.length && !allowCustomEntry ? /* @__PURE__ */ jsx(ComboBoxOption, {
				isStatic: true,
				isDisabled: true,
				label: "",
				value: "",
			...
			!optionsFilteredWithoutGroupLabels.length && allowCustomEntry && optionValueSelected !== filterValue ? /* @__PURE__ */ jsx(ComboBoxOption, {
				isStatic: true,
				isDisabled: true,
				label: "",
				value: "",
...
function TextInput({ className, clearIconRef, defaultValue, errorMessage, innerRef, id, isClearable, isDisabled, isInvalid, isLabelSkipped, isRequired, isShowingClearableIcon, label, labelClassName, name, onChange, onClear, onKeyUp, placeholder, rightContent, value, wrapperClassName, ...rest }) {
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		ref: innerRef,
		children: [
			isLabelSkipped ? null : /* @__PURE__ */ jsxs("label", {
				htmlFor: id,
				className: labelClassName ?? void 0,
				children: [label, isRequired ? /* @__PURE__ */ jsx(RequiredStar, {}) : null]
			}),
			/* @__PURE__ */ jsxs("div", {
				...
				children: [
					...
					showClearIcon ? /* @__PURE__ */ jsx(IconButton, {
...
function ComboBoxTextInput({ allowCustomEntry, className, comboBoxListId, errorMessage, iconCallback, id, innerRef: draftInnerRef, isClearable, isInvalid, isShowingClearableIcon, isDisabled, onBlur, onClear, onCustomEntry, onKeyUp, placeholder, firstSelectableByEnter, ...rest }) {
	...
	const onDownArrowPress = useOnKeyUp("ArrowDown", useCallback(() => {
	...
	const onEnterPress = useOnKeyUp("Enter", useCallback(
	...
	return /* @__PURE__ */ jsx("div", { children: /* @__PURE__ */ jsx(TextInput, {
		...
		innerRef: (ref) => {
			...
			if (draftInnerRef) if (isFunction(draftInnerRef)) draftInnerRef(input);
		...
		placeholder,
		rightContent: /* @__PURE__ */ jsx(IconButton, {
			className: "combo-box-input__chevron icon-button--borderless icon-button--small1x",
			icon: iconCallback?.(isOptionsExpanded) ?? /* @__PURE__ */ jsx("span", {
				"aria-hidden": "true",
...
function ComboBox({ allowCustomEntry, children, className, defaultValue, errorMessage, iconCallback, id, innerRef: draftInnerRef, isClearable, isDisabled, isInvalid, isRequired, isShowingClearableIcon, label, labelClassName, name, onChange, onCustomEntry, onClear, onKeyUp, placeholder, popupContentRef, portalTarget = null, isValueClearedOnSelection, isWrapperSkipped, tagChildren, textInputClassName, value, wrapperClassName, firstSelectableByEnter, ...rest }) {
	...
	const child = /* @__PURE__ */ jsxs("div", {
		...
		children: [
			tagChildren,
			/* @__PURE__ */ jsx(ComboBoxTextInput, {
				allowCustomEntry,
	...
	return /* @__PURE__ */ jsx(ComboBoxContextProvider, {
		...
		onKeyUp,
		value,
		children: isWrapperSkipped ? child : /* @__PURE__ */ jsx("div", {
...
function ComboBoxOptionGroupContextProvider({ children, optionGroupId }) {
	return /* @__PURE__ */ jsx(ComboBoxOptionGroupContext.Provider, {
		value: optionGroupId,
		children
	});
}
...
function ComboBoxOptionGroup({ children, className, label }) {
	...
	return /* @__PURE__ */ jsxs(ComboBoxOptionGroupContextProvider, {
		optionGroupId,
		children: [/* @__PURE__ */ jsx(ComboBoxOption, {
...
function DateInput({ ariaLabel, className, dateFormat, defaultValue, errorMessage, hasCalendarPopup = true, id, innerRef: draftInnerRef, isClearable, isDisabled, isRequired, label, labelClassName, name, onChange, onClear, onKeyUp, placeholder, showCalendarTodayButton, value, wrapperClassName, ...rest }) {
	...
	const onDownArrowPress = useOnKeyUp("ArrowDown", useCallback(() => setIsCalendarPopupOpen(true), []), true);
	return /* @__PURE__ */ jsx("div", {
		...
		children: /* @__PURE__ */ jsxs("div", {
			...
			children: [/* @__PURE__ */ jsx("div", { children: /* @__PURE__ */ jsx(TextInput, {
				...
				placeholder,
				value: value ?? "",
				rightContent: hasCalendarPopup ? /* @__PURE__ */ jsx(IconButton, {
					"aria-hidden": "true",
				...
				...rest
			}) }), hasCalendarPopup ? /* @__PURE__ */ jsx("div", {
				...
				children: /* @__PURE__ */ jsx(CalendarInput, {
					dateFormat,
					label,
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [
			...
			/* @__PURE__ */ jsxs("div", {
				...
				children: [
					/* @__PURE__ */ jsx("div", { className: "file-input__safari" }),
					!files?.length ? /* @__PURE__ */ jsx("div", {
					...
					files?.length ? /* @__PURE__ */ jsxs("div", {
						...
						children: [
							/* @__PURE__ */ jsxs("div", {
								...
								}), /* @__PURE__ */ jsxs("span", { children: ["Change file", files.length > 1 ? "s" : ""] })]
							}),
							/* @__PURE__ */ jsx("hr", {}),
							/* @__PURE__ */ jsx("div", {
								className: "file-input__file-list flex-wrap",
								children: [...files].map((file) => isFunction(children) ? /* @__PURE__ */ jsx("div", { children: children(file, removeFile) }, file.name) : /* @__PURE__ */ jsx(Tag, {
...
function MultiSelectClearIcon({ isClearable, isDisabled }) {
	...
	return isClearable && multiSelectContextValue.selectedValues.length ? /* @__PURE__ */ jsx(IconButton, {
...
function MultiSelectTags({ isDisabled }) {
	...
	return /* @__PURE__ */ jsx("ul", {
		...
		children: multiSelectContext.selectedValues.map((selectedValue, selectedValueIndex) => {
			...
			return selectedOption ? /* @__PURE__ */ jsx(MultiSelectTagWrapper, {
				selectedOption,
				selectedValueIndex,
				children: multiSelectContext.tagTemplate ? multiSelectContext.tagTemplate(selectedValue, selectedOption) : /* @__PURE__ */ jsx(Tag, {
		...
		children: [
			...
			/* @__PURE__ */ jsx("div", {
				...
				children: /* @__PURE__ */ jsxs("div", {
					...
					children: [
						/* @__PURE__ */ jsx(MultiSelectTags, { isDisabled }),
						...
						/* @__PURE__ */ jsx(MultiSelectClearIcon, {
							isClearable,
							isDisabled
						}),
...
function MultiSelectContextProvider({ children, multiSelectId, defaultValues, onChange, onClear, values }) {
	...
	const multiSelectContextNonStateRef = useRef({
		comboBoxDivElement: null,
		selectedOptionTagRefs: [],
		textInput: null
	});
	const multiSelectImmer = useImmer(() => ({
		clearButtonHasFocus: false,
		comboBoxOptions: [],
		focusedValueTagIndex: NaN,
		tagTemplate: null,
	...
	const providerValue = useMemo(() => [...multiSelectImmer, multiSelectContextNonStateRef], [multiSelectImmer]);
	return /* @__PURE__ */ jsx(MultiSelectContext.Provider, {
		value: providerValue,
		children
	});
}
...
function MultiSelect({ allowCustomEntry, children, className, defaultValues, errorMessage, id, innerRef, isClearable, isDisabled, isRequired, label, labelClassName, name, onChange, onClear, onCustomEntry, placeholder, values, wrapperClassName, ...rest }) {
	return /* @__PURE__ */ jsx(MultiSelectContextProvider, {
		defaultValues,
		multiSelectId: id,
		onChange,
		onClear,
		values,
		children: /* @__PURE__ */ jsx(MultiSelectComboBox, {
			allowCustomEntry,
			className,
			errorMessage,
			innerRef,
...
function MultiSelectOptionGroup({ children, label }) {
	return /* @__PURE__ */ jsx(ComboBoxOptionGroup, {
...
function PlainText({ className, innerRef, id, isLabelSkipped, label, labelClassName, value, wrapperClassName, ...rest }) {
	return /* @__PURE__ */ jsxs("div", {
		...
		ref: innerRef,
		children: [isLabelSkipped ? null : /* @__PURE__ */ jsx("div", {
			className: labelClassName ?? void 0,
			children: label
		}), /* @__PURE__ */ jsx("div", {
			...
			children: /* @__PURE__ */ jsx("div", {
				...
				id,
				...rest,
				children: value || /* @__PURE__ */ jsx(Fragment, { children: "\xA0" })
			})
		})]
	});
}
...
var RadioButtonGroupContext = createContext([void 0, () => {
...
function RadioButtonGroupContextProvider({ children, defaultValue, name, onChange, value }) {
	const radioButtonGroupImmer = useImmer({
		name,
		onChange: (newValue) => {
			if (onChange) onChange(newValue);
...
function RadioButtonGroup({ children, className, defaultValue, errorMessage, id, isRequired, label, onChange, value }) {
	return /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsxs("fieldset", {
		id,
		className: joinClassNames("fieldset fieldset--radio-wrapper", className),
		children: [/* @__PURE__ */ jsxs("legend", { children: [label, isRequired ? /* @__PURE__ */ jsx(RequiredStar, {}) : null] }), /* @__PURE__ */ jsx(RadioButtonGroupContextProvider, {
			defaultValue,
			name: id,
			onChange,
			value,
		...
		children: [
			...
			/* @__PURE__ */ jsxs("div", {
				...
				children: [/* @__PURE__ */ jsxs("select", {
					...
					value,
					...rest,
					children: [placeholder ? /* @__PURE__ */ jsx(SelectOption, {
						label: placeholder,
						value: "",
						isDisabled: true
					}) : null, children]
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [/* @__PURE__ */ jsxs("label", {
			...
			children: [
				...
				labelOn || labelOff ? /* @__PURE__ */ jsxs("span", {
...
function TimeInput({ allowCustomEntry, className, defaultValue, errorMessage, hasTimePopup = true, id, innerRef, isClearable, isDisabled, isRequired, label, labelClassName, name, onChange, onClear, placeholder, timeFormat = "h:mm aaa", timeRangeBegin, timeRangeEnd, timeRangeIncrement = 15, value, wrapperClassName, ...rest }) {
	const timeOptions = useMemo(() => {
		...
		for (let loopDate = optionsBeginDate; loopDate.getTime() <= optionsEndDate.getTime(); loopDate = add(loopDate, { minutes: timeRangeIncrement })) timeOptionsRet.push(format(loopDate, timeFormat));
	...
	const clockIcon = useMemo(() => /* @__PURE__ */ jsx("span", {
	...
	return /* @__PURE__ */ jsx("div", {
		...
		ref: innerRef,
		children: hasTimePopup ? /* @__PURE__ */ jsx(ComboBox, {
			className,
			defaultValue,
			errorMessage,
			id,
			...
			onChange,
			...rest,
			children: timeOptions.map((timeOption) => /* @__PURE__ */ jsx(ComboBoxOption, {
				label: timeOption,
				value: timeOption
...
function MenuItemInline({ currentMenuItem, expandChildrenByDefault = true, menuItem, menuType = menuTypes.VERTICAL }) {
	...
	return /* @__PURE__ */ jsxs("li", {
		...
		children: [/* @__PURE__ */ jsxs("span", {
			...
			children: [
				!menuItem?.link && !menuItem.actionFunction && !menuItem.actionFunctionUrl && !menuItem.actionUrl || menuItem?.link?.includes("::") ? /* @__PURE__ */ jsx("button", {
				...
				menuItem.children ? /* @__PURE__ */ jsx(IconButton, {
					appearance: ICON_BUTTON_APPEARANCE.BORDERLESS,
				...
				/* @__PURE__ */ jsx("span", { className: "menu-chiclet" })
			]
		}), menuItem.children ? /* @__PURE__ */ jsx("ul", {
...
function HorizontalMenu({ className, currentMenuItem, id, menu, titleTagClassName = "visually-hidden", titleTagName: TitleTagName = "h2" }) {
	return /* @__PURE__ */ jsxs("nav", {
		...
		"aria-labelledby": id,
		children: [/* @__PURE__ */ jsx(TitleTagName, {
			id,
			className: titleTagClassName,
			children: "Main Menu"
		}), /* @__PURE__ */ jsx("ul", { children: menu?.menuItems?.map((menuItem) => /* @__PURE__ */ jsx(MenuItemInline, {
			menuItem,
			currentMenuItem
...
function findElementsByTagNameMatch(element) {
	return notNullArray([element?.tagName?.match?.(/^h[23]$/i) ? element : null, ...element?.children ? Array.from(element.children).map((child) => findElementsByTagNameMatch(child)) : []].flat(Infinity).filter(identity).filter((elementMaybeHasId) => elementMaybeHasId?.id), "finedElementsByTagNameMatch: how did a null slip by the identity");
}
//#endregion
...
function MenuItemPlain({ currentMenuItem, menuItem, menuType }) {
	return /* @__PURE__ */ jsxs("li", {
		...
		children: [/* @__PURE__ */ jsxs("span", {
			...
			children: [!menuItem?.link && !menuItem.actionFunction && !menuItem.actionFunctionUrl && !menuItem.actionUrl || menuItem?.link?.includes("::") ? /* @__PURE__ */ jsx("span", {
		...
		}), menuItem.children?.length ? /* @__PURE__ */ jsx("ul", {
			className: menuType === menuTypes.VERTICAL ? "vertical-menu" : "",
			children: menuItem.children.map((menuItemChild) => /* @__PURE__ */ jsx(MenuItemPlain, {
				currentMenuItem,
				menuItem: menuItemChild,
				menuType
			}, `menu-item__child__${menuItemChild.link}-${menuItemChild.title}}`))
...
function MenuItemFlyout({ currentMenuItem, menuItem, menuType, triggerOnHover = true }) {
	...
	useEffect(() => {
		if (triggerOnHover && buttonRef?.current && popupRef?.current && !buttonRef?.current.onclick) popupFocusHandler(wrapperElement.current, buttonRef.current, popupRef.current, "menu", {
	...
	return /* @__PURE__ */ jsxs("li", {
		...
		children: [/* @__PURE__ */ jsxs("span", {
			className: menuType === menuTypes.VERTICAL ? "vertical-menu__title" : "menu-item__title",
			children: [
				!menuItem?.link && !menuItem.actionFunction && !menuItem.actionFunctionUrl && !menuItem.actionUrl || menuItem?.link?.includes("::") ? /* @__PURE__ */ jsxs("button", {
					...
					children: [/* @__PURE__ */ jsx("span", {
						...
						children: menuItem.title
					}), menuItem.children ? /* @__PURE__ */ jsx("span", {
				...
				menuItem.children && menuItem.link ? /* @__PURE__ */ jsx(IconButton, {
					appearance: ICON_BUTTON_APPEARANCE.BORDERLESS,
					"aria-labelledby": encodeURI(`menu-item-${menuItem.id}-${menuItem.link || "link"}`),
					"aria-expanded": isChildrenOpen ? "true" : "false",
		...
		}), menuItem.children ? /* @__PURE__ */ jsx("div", {
			"aria-labelledby": encodeURI(`menu-item-${menuItem.id}-${menuItem.link || "link"}`),
			...
			children: /* @__PURE__ */ jsx("div", {
				...
				children: /* @__PURE__ */ jsx("ul", {
					className: menuType === menuTypes.VERTICAL ? "vertical-menu" : "",
					children: menuItem.children?.map((menuItemChild) => /* @__PURE__ */ jsx(MenuItemFlyout, {
						currentMenuItem,
						menuItem: menuItemChild,
						menuType,
						triggerOnHover
...
function VerticalMenu({ className, currentMenuItem, expandInlineChildrenByDefault = true, menus, triggerOnHover = true }) {
	return /* @__PURE__ */ jsx(Fragment, { children: menus.map((menu) => {
		...
		return /* @__PURE__ */ jsxs("div", {
			...
			children: [menu.header ? /* @__PURE__ */ jsx(TitleTagName, {
				id: menu.id,
				...
				children: menu.menuItems.map((menuItem) => {
					...
					switch (menuItem.childrenMenuType) {
						...
						case childrenMenuTypes.FLYOUT:
							result = /* @__PURE__ */ jsx(MenuItemFlyout, {
								menuType: menuTypes.VERTICAL,
								currentMenuItem,
								menuItem,
								triggerOnHover
							...
							break;
						default: result = /* @__PURE__ */ jsx(MenuItemPlain, {
							menuType: menuTypes.VERTICAL,
							menuItem,
							currentMenuItem
						}, `vertical-menu__menu-item__${menuItem.link}-${menuItem.title}}`);
...
function Pagination({ ariaLabel, className, defaultValue = 0, id, innerRef, onChange, itemsPerPage, totalNumberItems, value = 0, wrapInElement = "div", ...rest }) {
	const [currentValue, setCurrentValue] = useImmer(value === void 0 ? defaultValue : value);
	const valueUse = value === void 0 ? currentValue : value;
	const valueRef = useRefAlways(valueUse ?? 0);
	const { onChange: currentOnChange, value: currentPageIndex, valueRef: currentPageIndexRef } = useMemo(() => ({
	...
	const paginationLinks = useMemo(() => determinePaginationLinks({
		currentPageIndex,
		numberOfPages
	}), [currentPageIndex, numberOfPages]);
	return /* @__PURE__ */ jsx(wrapInElement || "div", {
		ref: innerRef,
		...
		children: /* @__PURE__ */ jsxs("ul", { children: [
			/* @__PURE__ */ jsx("li", { children: /* @__PURE__ */ jsx(IconButton, {
				appearance: ICON_BUTTON_APPEARANCE.BORDERLESS,
			...
			paginationLinks.map((paginationLink, paginationLinkIndex) => paginationLink.isEllipsis ? /* @__PURE__ */ jsx("li", { children: /* @__PURE__ */ jsx("span", {
				className: "pagination__ellipsis",
				children: /* @__PURE__ */ jsx("span", {
					className: "utds-icon-before-more-horizontal",
					"aria-hidden": "true"
				})
			}) }, `pagination-link__ellipsis__${paginationLinkIndex}`) : /* @__PURE__ */ jsx(PaginationLink, {
				currentPageIndex,
...
* @returns {import('react').JSX.Element}
*/
function Popup({ ariaLabelledBy, children, className, hasCloseButton, id, innerRef: draftInnerRef, isVisible, offset: offset$1 = {
	mainAxis: 10,
	crossAxis: 0
...
*/
/** @template TableDataT */
var TableBodyDataRowContext = createContext({ record: null });
//#endregion
...
function createTableFilterFunctions(filterValues) {
	return Object.fromEntries(Object.entries(filterValues || {}).map(([filterKey, filterData]) => {
		...
		if (filterData.options?.exactMatch) {
			...
			const beginDateDate = beginDate ? parse(beginDate, dateFormat, /* @__PURE__ */ new Date()) : null;
			const endDateDate = endDate ? parse(endDate, dateFormat, /* @__PURE__ */ new Date()) : null;
			testFunc = (value) => {
...
function TableBodyData({ children, recordIdField, records }) {
	...
	useEffect(() => {
		...
		if (tableSortingFieldPath || tableSortingFieldPaths) {
			const sorters = castArray(tableSortingFieldPaths || tableSortingFieldPath).map((sortingValue) => sortingRules[sortingValue ?? ""]?.sorter).filter(identity).map(notNullMap);
	...
	return recordsForContexts?.length ? paginatedRecords?.map((recordForContext) => /* @__PURE__ */ jsx(TableBodyDataRowContext.Provider, {
...
function TableBodyDataCellTemplate({ children, className, id, innerRef, onClick, onDoubleClick, recordFieldPath, ...rest }) {
	...
	let content;
	if (isFunction(children)) content = children(rowContextData);
...
function TableBodyDataRowTemplate({ children, className, innerRef, onClick, onDoubleClick, ...rest }) {
	...
	return /* @__PURE__ */ jsx(TableRow, {
		className: joinClassNames(isFunction(className) ? className(rowContextData) : className),
		onClick: (onClick && ((e) => onClick({
			e,
			...rowContextData
		}))) ?? void 0,
...
function setValueAtPath({ object, path, value }) {
	...
	if (path) {
		const targetObject = pathPieces.reduce((draftNextLevel, pathPiece) => {
			...
			if (isObject(draftNextLevel)) {
				...
				if (childObj === void 0 || childObj === null) {
					...
					if (isArray(childObj)) childObj = childObj.concat([]);
		...
		if (targetObject && isObject(targetObject)) targetObject[field] = valueCloned;
...
(newValue) => newValue),
	...
	const dataOptions = useMemo(() => uniq(allData.map((datum) => datum[recordFieldPath]).filter(identity)).sort(chainSorters([(a, b) => a < b ? -1 : 0, (a, b) => a > b ? 1 : 0])), [allData]);
	useTableFilterRegistration(recordFieldPath, defaultValue, { exactMatch: !!exactMatch });
	return /* @__PURE__ */ jsx("th", {
		className: joinClassNames("table-header__cell table-header__cell--filter-combo-box", className),
		id: id ?? void 0,
		...
		children: /* @__PURE__ */ jsxs(ComboBox, {
			...
			children: [/* @__PURE__ */ jsx(ComboBoxOption, {
				...
				value: "",
				...rest
			}), dataOptions.map((dataOption) => /* @__PURE__ */ jsx(ComboBoxOption, {
				className: className ?? void 0,
				label: dataOption,
				value: dataOption,
				...rest
...
function TableFilterDateRangeButtonTitle({ currentValue, placeholder }) {
	...
	return foundMatch ? /* @__PURE__ */ jsxs(Fragment, { children: [
		beginDate,
		...
		endDate
	] }) : /* @__PURE__ */ jsx(Fragment, { children: placeholder ?? "Filter" });
}
//#endregion
...
function TableFilterDateRangePopup({ dateFormat, id, isPopupOpen, onChange, popupReferenceElement, setIsPopupOpen, tableFilterDateId, value }) {
	...
	useGlobalKeyEvent({
		whichKeyCode: "Escape",
		onKeyUp: useCallback(() => setIsPopupOpen(false), [])
	});
	...
	return /* @__PURE__ */ jsxs(Popup, {
		...
		children: [/* @__PURE__ */ jsxs("div", {
			...
			/* @__PURE__ */ jsxs("div", {
				...
				children: [/* @__PURE__ */ jsx("div", { className: joinClassNames("table-filter-date-popup__selected-date-chiclet", currentInput === BeginEndDates.BEGIN ? "table-filter-date-popup__selected-date-chiclet--selected" : "table-filter-date-popup__selected-date-chiclet--not-selected") }), /* @__PURE__ */ jsx("div", { className: joinClassNames("table-filter-date-popup__selected-date-chiclet", currentInput === BeginEndDates.END ? "table-filter-date-popup__selected-date-chiclet--selected" : "table-filter-date-popup__selected-date-chiclet--not-selected") })]
			}),
			/* @__PURE__ */ jsx("hr", {}),
...
(newValue) => newValue),
	...
	return /* @__PURE__ */ jsxs("th", {
		...
		children: [/* @__PURE__ */ jsxs("div", {
			...
			children: [
				/* @__PURE__ */ jsx(Button, {
					...
					children: /* @__PURE__ */ jsx(TableFilterDateRangeButtonTitle, {
						currentValue,
						placeholder
					})
				}),
				currentValue && currentValue !== tableConstants.dateFilterSeparator ? /* @__PURE__ */ jsx(IconButton, {
					className: joinClassNames("text-input__clear-button icon-button--borderless icon-button--small1x"),
					icon: /* @__PURE__ */ jsx("span", {
						className: "utds-icon-before-x-icon",
						"aria-hidden": "true"
		...
		}), /* @__PURE__ */ jsx(TableFilterDateRangePopup, {
			dateFormat,
			id: popupId,
	...
	return /* @__PURE__ */ jsx("th", {
		...
		children: /* @__PURE__ */ jsxs(Select, {
			...
			children: [/* @__PURE__ */ jsx(SelectOption, {
				...
				value: "",
				...rest
			}), dataOptions.map((dataOption) => /* @__PURE__ */ jsx(SelectOption, {
				className: className ?? void 0,
				label: dataOption,
				value: dataOption,
				...rest
...
function TableHeadCell({ children, className, id, innerRef, onClick, recordFieldPath, scope, tableSortingFieldPaths, ...rest }) {
	...
	return /* @__PURE__ */ jsx("th", {
		...
		children: isSortable ? /* @__PURE__ */ jsxs("button", {
			onClick: onClickCallback,
			type: "button",
			children: [isCurrentSortingField ? /* @__PURE__ */ jsxs("span", {
				className: "visually-hidden",
				children: ["Currently sorted by ", mySortingRules?.filter(identity)?.map((sortingRule) => {
...
function Badge({ children, className, innerRef, title, ...rest }) {
	return /* @__PURE__ */ jsxs("div", {
		className: joinClassNames("badge", className, isEmpty(children) ? "badge--blank" : ""),
		ref: innerRef,
		...rest,
		children: [/* @__PURE__ */ jsx("span", {
...
function AriaLiveMessages({ ariaLiveType, messages }) {
	const messagesLists = useMemo(() => messages.reduce((lists, message, i) => {
	...
	return /* @__PURE__ */ jsx("div", {
		...
		children: messagesLists.map((messagesList) => messagesList[messagesList.length - 1]).map((messagesListMessage, i) => messagesListMessage ? /* @__PURE__ */ jsx("div", {
			"aria-live": ariaLiveType,
			role,
			children: messagesListMessage.message
		}, `${ariaLiveType}-messages-${i}`) : /* @__PURE__ */ jsx("div", {
			"aria-live": ariaLiveType,
			role
...
function BannersGlobal({ banners, bannerDuration, defaultClassName }) {
	...
	return /* @__PURE__ */ jsx("div", {
		...
		"aria-live": "polite",
		children: Object.keys(zones).map((zone) => /* @__PURE__ */ jsx("div", {
			className: joinClassNames(`banner-global__${zone}`, "banner-global__zone"),
			children: zones[zone]?.map((banner) => /* @__PURE__ */ jsxs(Banner, {
				id: `banner__${banner.id}`,
				className: banner.className || defaultClassName,
				position: banner.position,
				onClose: (e) => currentOnClose(e, banner),
				children: [banner.icon ? /* @__PURE__ */ jsx(BannerIcon, { children: banner.icon }) : "", /* @__PURE__ */ jsx(BannerMessage, { children: banner.message })]
			}, `banner__${banner.id}`))
...
function UtahDesignSystemContextProvider({ children, defaultSettings }) {
	...
	return /* @__PURE__ */ jsxs(UtahDesignSystemContext.Provider, {
		value: immerHook,
		children: [
			children,
			/* @__PURE__ */ jsx(AriaLiveMessages, {
...
var UtahHeaderContext = createContext({
...
function UtahHeaderContextProvider({ children, defaultSettings }) {
	...
	return /* @__PURE__ */ jsx(UtahHeaderContext.Provider, {
...
//#endregion
export { Accordion, BANNER_PLACEMENT, BUTTON_APPEARANCE, BUTTON_TYPES, Badge, Banner, BannerIcon, BannerMessage, Button, CalendarInput, CharacterCount, Checkbox, ClickableTag, ComboBox, ComboBoxOption, ComboBoxOptionGroup, ConfirmationButton, ConfirmationChildren, DRAWER_PLACEMENT, DateInput, DocumentationTemplate, Drawer, DrawerContent, DrawerFooter, DrawerTitle, ExternalLink, FileInput, FooterAgencyInformation, FooterAgencyInformationColumn, FooterAgencyInformationInfo, FooterSocialMediaBar, Form, HorizontalMenu, ICON_BUTTON_APPEARANCE, IconButton, InitialChildren, LandingTemplate, LinkCallback, MainContent, Modal, ModalContent, ModalFooter, ModalTitle, MultiSelect, MultiSelectOption, MultiSelectOptionGroup, MultiSelectTagTemplate, OnThisPage, Pagination, PlainText, Popup, RadioButton, RadioButtonGroup, Select, SelectOption, Skeleton, Spinner, Switch, Tab, TabGroup, TabGroupTitle, TabList, TabPanel, TabPanels, Table, TableBody, TableBodyData, TableBodyDataCellTemplate, TableBodyDataRowTemplate, TableCell, TableContextConsumer, TableFilterComboBox, TableFilterComboBoxAllOptions, TableFilterComboBoxOption, TableFilterCustom, TableFilterDateRange, TableFilterNone, TableFilterSelect, TableFilterSelectAllOptions, TableFilterSelectOption, TableFilterTextInput, TableFilters, TableFoot, TableFootCell, TableFootRow, TableHead, TableHeadCell, TableHeadRow, TablePagination, TableRow, TableSortingRule, TableSortingRules, TableWrapper, Tag, TextArea, TextInput, TimeInput, Tooltip, UtahDesignSystemContext, UtahDesignSystemContextProvider, UtahHeaderContext, UtahHeaderContextProvider, VerticalMenu, chainSorters, componentColors, events, formElementSizesEnum, getFocusableElements, handleEvent, handleKeyPress, joinClassNames, menuTypes, popupPlacement, rectContainsPoint, renderDOMSingle, setValueAtPath, skeletonTypes, stringToId, tableSortingRuleFieldType, useAriaMessaging, useBanner, useGlobalKeyEvent, useHandleEscape, useHandleTab, useImmerRef, useInterval, useMountingTracker, usePaginatedList, usePopupDelay, useRefAlways, useRefLazy, useTimeout, useUtahDesignSystemContext, useUtahHeaderContext, valueAtPath, version };
